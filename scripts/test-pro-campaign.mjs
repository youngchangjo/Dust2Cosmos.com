...
const valid = {schemaVersion:1,campaignID:'pro-3-0-launch',enabled:true,productID:'com.dusttocosmos.app.pro',minimumAppVersion:'3.0',startsAt:'2026-10-01T00:00:00Z',endsAt:'2026-10-15T00:00:00Z',markets:[{storefront:'KOR',currency:'KRW',price:'9900',regularPrice:'14900',timeZone:'Asia/Seoul'}]};
test('exact shared launch window, independent of browser installation', () => {
 ...
 assert.ok(campaignMarket(valid,'KOR',start+14*86400000-1));
 assert.equal(campaignMarket(valid,'KOR',start+14*86400000),null);
});
test('inactive, unknown storefront, malformed date, false discounts and duplicate markets stay hidden', () => {
 for(const patch of [{enabled:false},{startsAt:null},{endsAt:'not-a-date'},{productID:'other'},{minimumAppVersion:'4.0'},{markets:[valid.markets[0],valid.markets[0]]},{markets:[{...valid.markets[0],regularPrice:'9000'}]},{markets:[{...valid.markets[0],timeZone:'not-a-zone'}]}])assert.equal(campaignMarket({...valid,...patch},'KOR',start),null);
 assert.equal(campaignMarket(valid,'USA',start),null);
});
test('published configuration has no active sale or invented launch date', async () => {
 const c = JSON.parse(await readFile(new URL('../assets/pro-launch-campaign.json',import.meta.url)));
 assert.equal(c.enabled,false);assert.equal(c.startsAt,null);assert.equal(c.endsAt,null);assert.deepEqual(c.markets,[]);
});
...
test('real banner renders from server time, rejects failures, and clears on background', async () => {
 ...
 assert.match(fields.get('[data-campaign-title]').textContent,/2주/);
